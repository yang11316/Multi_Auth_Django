    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user_instance == None:
                return JsonResponse(
                    {"status": "add error failed", "message": "user not exist"}
                )
            ...
            for tmp in location_data:
                if RegisterSoftwareLocationTable.objects.filter(
                    entity_ip=tmp["entity_ip"],
                    rsoftware_id=rsoftware_instace.rsoftware_id,
                ).exists():
                    continue
    ...
    if request.method == "POST":
        try:
            ...
            if software_instance == None:
                return JsonResponse(
                    {"status": "error", "message": "software not exist"}
                )
...
def software_add(request):
    if request.method == "POST":
        ...
        try:
            ...
            # 应该加上一个路经检测，后期加
            software_instace.software_hash = calculate_file_hash(
            ...
            for (
                rlsoftwarelocation_instance
            ) in RegisterSoftwareLocationTable.objects.filter(
                rsoftware_id=rsoftware_id
            ):
    ...
    if request.method == "POST":
        try:
            ...
            if not query_instance:
                return JsonResponse(
                    {"status": "error", "message": "software_id not exist"}
                )
            data = query_instance.get_data()
            return JsonResponse({"status": "success", "message": data})
        except Exception as e:
    ...
    if request.method == "POST":
        try:
            ...
            if is_approve:
                ...
                software_instance.software_version = (
                    rsoftware_instance.rsoftware_version
                )
                ...
                rsoftwarelocation_instace = (
                    RegisterSoftwareLocationTable.objects.filter(
                        rsoftware_id=rsoftware_id
                    )
                )
                ...
                rlsoftwarelocation_instance_list = (
                    RegisterSoftwareLocationTable.objects.filter(
                        rsoftware_id=rsoftware_id
                    )
                )
