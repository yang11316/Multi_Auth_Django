...
bool socket_send(const std::string &message, const std::string &send_ip, const unsigned short send_port, const unsigned short sending_port, const std::string &local_ip)
{
    ...
    {
        std::cerr << "Error setting reuse address: " << std::strerror(errno) << std::endl;
        return false;
    }
    ...
    local_addr.sin_addr.s_addr = inet_addr(local_ip.data()); // 本地地址
...
void deal_httpmsg(std::string &recv_data, int client_socket)
{
    ...
    // 如果长度是1则为发布的aux消息
    if (params.size() == 1 && params.count("aux"))
    {
    ...
    else if (params.size() == 4)
    {
        ...
        {
            process.init(params["pid"], params["acc_publickey"], params["acc_cur"], params["kgc_Ppub"], 714);
        }
    }
    else if (params.size() == 5)
    {
        ...
        {
            process.init(params["pid"], params["acc_publickey"], params["acc_cur"], params["entity_parcialkey"], params["kgc_Ppub"], 714);
...
void send_sign_msg(const std::string &msg, const std::string &send_ip, const unsigned short send_port, const unsigned short sending_port, const std::string &local_ip)
{
    if (!process.is_fullkey)
    {
    ...
    if (socket_send(message, send_ip, send_port, sending_port, local_ip))
    {
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 9)
    {
        std::cerr << "Usage: " << argv[0] << " <message> <listening_ip> <listening_port> <sending_port> <ap_ip> <ap_port> <send_ip> <send_port>" << std::endl;
        return 1;
    }
    // 从命令行参数中获取信息
    std::string entity_pid = argv[1];
    std::string listening_ip = argv[2];
    unsigned short listening_port = std::stoi(argv[3]);
    unsigned short sending_port = std::stoi(argv[4]);
    std::string ap_ip = argv[5];
    unsigned short ap_port = std::stoi(argv[6]);
    std::string send_ip = argv[7];
    unsigned short send_port = std::stoi(argv[8]);

    // 输出获取到的信息
    std::cout << "entity_pid: " << entity_pid << std::endl;
    ...
    send_http_msg(entity_pid, ap_ip, ap_port, listening_port, sending_port, listening_ip);
    if (send_ip.compare("0"))
    {
        sleep(2);
        send_sign_msg("hello", send_ip, send_port, sending_port, listening_ip);
    }
