...
bool Process::generate_full_key()
{
    ...
    this->public_key = std::make_tuple(X, WIT, wit_hash);
...
sign_payload Process::sign(const std::string &msg)
{
    ...
    // u = hash(m,PID,PK,t,Y)
    std::string u_str = msg + this->pid + crypto_utils::point2hex(this->ec_group, std::get<0>(this->public_key)) + crypto_utils::point2hex(this->ec_group, std::get<1>(this->public_key)) + crypto_utils::bn2hex(std::get<2>(this->public_key)) + ti + crypto_utils::point2hex(this->ec_group, Y);
    BIGNUM *u = crypto_utils::string2hash2bn(u_str);

    // h3 = hash(m,pid,pk,t)
    std::string h3_str = msg + this->pid + crypto_utils::point2hex(this->ec_group, std::get<0>(this->public_key)) + crypto_utils::point2hex(this->ec_group, std::get<1>(this->public_key)) + crypto_utils::bn2hex(std::get<2>(this->public_key)) + ti;
    ...
    // h3*x
    BN_mod_mul(w, h3, this->secret_key.first, this->q, bn_ctx);
    // u*y + h3*x
    BN_mod_add(w, w, tmp, this->q, bn_ctx);
    // h4*wit
    BN_mod_mul(tmp, h4, this->secret_key.second, this->q, bn_ctx);
    // w = u*y + h3*x + h4*wit
    BN_mod_add(w, w, tmp, this->q, bn_ctx);
    sign_payload payload = {
        this->pid,
        msg,
        crypto_utils::point2hex(this->ec_group, std::get<0>(this->public_key)),
        crypto_utils::point2hex(this->ec_group, std::get<1>(this->public_key)),
        crypto_utils::bn2hex(std::get<2>(this->public_key)),
...
bool Process::verify_sign(const sign_payload &payload)
{
    ...
    else
    {
        ...
        EC_POINT_free(WIT);
        BN_CTX_free(bn_ctx);
        std::cout << "[SUCESS]sign verifid sucessfully" << std::endl;
        return true;
    }
}