...
if __name__ == "__main__":
    ...
    # 生成CA自签名证书
    ca_cert = generate_self_signed_certificate(ca_private_key, "ca")
    # 生成CA私钥文件
    save_key_to_pem_file(ca_private_key, "ca.key")
    # 生成CA证书文件
    save_cert_to_pem_file(ca_cert, "ca.crt")
    """

    ca_private_key = load_key_from_pem_file("CA.key")
    ca_cert = load_cert_from_pem_file("CA.crt")
    print(load_cert_as_string(ca_cert))

"""
    # 生成用户1私钥
    user1_private_key = generate_private_key()
    # 生成用户1证书签名请求
    user1_csr = generate_certificate_signing_request(user1_private_key, "AS_cert")
    # 使用CA证书签署用户1证书签名请求
    user1_cert = sign_certificate(user1_csr, ca_cert, ca_private_key)
    # 生成用户1私钥文件
    save_key_to_pem_file(user1_private_key, "AS_PKI.key")
    # 生成用户1证书文件
    save_cert_to_pem_file(user1_cert, "AS_PKI.crt")

    # 生成用户2私钥
    user2_private_key = generate_private_key()
    # 生成用户2证书签名请求
    user2_csr = generate_certificate_signing_request(user2_private_key, "AP_cert")
    # 使用CA证书签署用户2证书签名请求
    user2_cert = sign_certificate(user2_csr, ca_cert, ca_private_key)
    # 生成用户2私钥文件
    save_key_to_pem_file(user2_private_key, "AP_PKI.key")
    # 生成用户2证书文件
    save_cert_to_pem_file(user2_cert, "AP_PKI.crt")

    # 用户1验证用户2的证书
    # ca_cert = load_cert_from_pem_file("ca.crt")
    # user2_cert = load_cert_from_pem_file("user2.crt")
    # 验证用户2的证书
    if verify_certificate(user2_cert, ca_cert):
        print("User2 certificate is valid.")
    else:
        print("User2 certificate is invalid.")
"""
